          ...
          <div className="border border-[#111]/10 divide-y divide-[#111]/10">
            {aboutContent.values.map((item, i) => (
              <div key={i} className="p-8 lg:p-10 hover:bg-[#F5F5F5] transition-colors">
                ...
                <h3 className="text-xl font-medium uppercase tracking-tight text-[#111] mb-3">{dynamicT(item.title)}</h3>
                <p className="text-sm font-body text-[#111]/60 leading-relaxed">{dynamicT(item.desc)}</p>
              </div>
            ))}
          </div>
        </div>
